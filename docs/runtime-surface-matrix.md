...
Last reviewed: 2026-08-05. This matrix separates repository and lifecycle evidence from claims that require a request-level Codex runtime observation. `VERIFIED` means a repository Gate or linked sanitized record directly covers the stated surface; `PARTIAL` means only a narrower behavior is covered; `UNVERIFIED` means no public request-level evidence is available; `OUT OF SCOPE` is deliberately not a current promise.

| Surface or claim | Status | Evidence and boundary |
| --- | --- | --- |
| Both canonical Skills and UI metadata shapes | VERIFIED | `tests/test_contract.py`, `tests/test_heavy_contract.py`, and source-suite validation cover both public runtimes. |
| Adaptive/heavy/all install, replace, rollback, validate, and fail-closed uninstall | VERIFIED | `tests/test_install.py` covers manifest v2, v1 adaptive reads, exact trees, shared locking, suite transactions, rollback seams, and cleanup debt. |
...
| D1 opt-in single-worker contract | VERIFIED | Schema-v3 adaptive fixtures and contract tests cover positive and fail-closed D1 decisions; request-level execution remains unverified. |
...
| L3 `l3-v1` handoff contract and digest fixtures | VERIFIED | `tests/test_l3_handoff.py` covers exact packet identity, owner states, ready and fail-closed outcomes. |
| Bundled heavy workflow and scaffold | VERIFIED | Static contract and scaffold tests cover architecture-first phases, recovery, layered QA, manual Gates, dry-run, no-overwrite, and path safety. |
| Request-level adaptive → heavy execution | PARTIAL | A representative local Flow produced the integrated architecture, contracts, DAG, status, QA, and readiness artifacts; universal host discovery and request identity remain unverified. |
...
| Windows runtime | OUT OF SCOPE | No native Windows lifecycle implementation or Gate is claimed for the current candidate. |
